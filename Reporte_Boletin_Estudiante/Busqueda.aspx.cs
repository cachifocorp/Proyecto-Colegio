...
public partial class Reporte_Boletin_Busqueda : System.Web.UI.Page
{
    ...
    public bool datos_base(int id_usuario) {
       // ConexionDB db = new ConexionDB(ConfigurationManager.ConnectionStrings["estigioacademicoConnectionString"].ConnectionString);
        clsDb db = new clsDb();
        String SQL = "SELECT es.[id] ,es.[documento_numero], ma.id_salon, us.usuario"+
                     " FROM [estudiante] es"+
                     " inner join matricula ma on es.id = ma.id_estudiante "+
                     " inner join  usuario us on es.documento_numero = us.documento"+
        ...
        try
        {
            ...
            while (reader.Read()){
                this.Id_alumno = Convert.ToInt32(reader["documento_numero"].ToString());
                this.Id_salon = int.Parse(reader["id_salon"].ToString());
            }
            con.Close();
            return true;
        }catch(Exception ex){
    ...
    public void vertbl_Estudiante()
    {
        try
        {
            Asignacion objAsignacion = new Asignacion();
            OperacionAsignacion objOperAsignacion = new OperacionAsignacion(ConfigurationManager.ConnectionStrings["estigioacademicoConnectionString"].ConnectionString);
            objAsignacion.id_salon = this.Id_salon;
    ...
    {
        if (datos_base(int.Parse(Session["id_usuario"].ToString())))
        {
            //message.InnerHtml = "<p> des de boton  " + Id_alumno + " y esto tambien : "+Id_salon+"</p>";
            generarBoletin(Id_salon, Id_alumno);
        }
        else
        {
            message.InnerHtml = "<p>Error! Por favor Comuníquese con el Administrador</p>";
        }
